...
//
// Reference dequantizer for IQ3_S (3.4375 bpw) GGUF tensor type.
// Block = 110 bytes for 256 weights:
//   d (f16), qs[64], qh[8], signs[32], scales[4]
//
...
static constexpr int IQ3_S_BLOCK_BYTES = 110;

static const uint8_t kmask_iq2xs_iq3s[8] = {1, 2, 4, 8, 16, 32, 64, 128};

// IQ3_S grid: 512 entries (uint32_t), each encodes 4 weight magnitudes (bytes)
...
ov::Tensor dequantize_iq3_s(gguf_tensor* tensor) {
    OPENVINO_ASSERT(tensor->type == GGUF_TYPE_IQ3_S,
                    "[load_gguf] dequantize_iq3_s wrong tensor type: ", tensor->type);
    const uint64_t num_weights = tensor->num_weights;
    OPENVINO_ASSERT(num_weights % QK_K == 0,
                    "[load_gguf] IQ3_S num_weights not divisible by 256: ", num_weights);
    ...
    for (uint64_t b = 0; b < num_blocks; ++b) {
        const uint8_t* bd = raw + b * IQ3_S_BLOCK_BYTES;
        uint16_t d_fp16; memcpy(&d_fp16, bd, 2);
        const float d = fp16_to_fp32_k(d_fp16);
        const uint8_t* qs = bd + 2;        // 64 bytes
        const uint8_t* qh = bd + 66;       // 8 bytes
        const uint8_t* signs = bd + 74;    // 32 bytes
        const uint8_t* scales = bd + 106;  // 4 bytes
        ...
        for (int ib32 = 0; ib32 < QK_K / 32; ib32 += 2) {
            ...
            for (int l = 0; l < 4; ++l) {
                ...
                for (int j = 0; j < 4; ++j) {
                    out[j + 0] = fp32_to_fp16_k(db1 * g1[j] * ((signs[l] & kmask_iq2xs_iq3s[j + 0]) ? -1.f : 1.f));
                    out[j + 4] = fp32_to_fp16_k(db1 * g2[j] * ((signs[l] & kmask_iq2xs_iq3s[j + 4]) ? -1.f : 1.f));
                }
                out += 8;
            }
            qs += 8;
            ...
            for (int l = 0; l < 4; ++l) {
                ...
                for (int j = 0; j < 4; ++j) {
                    out[j + 0] = fp32_to_fp16_k(db2 * g1[j] * ((signs[l] & kmask_iq2xs_iq3s[j + 0]) ? -1.f : 1.f));
                    out[j + 4] = fp32_to_fp16_k(db2 * g2[j] * ((signs[l] & kmask_iq2xs_iq3s[j + 4]) ? -1.f : 1.f));
                }
                out += 8;
            }
            qh += 2;
