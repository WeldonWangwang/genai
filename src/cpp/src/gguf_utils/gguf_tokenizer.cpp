...
std::map<std::string, GGUFMetaData> tokenizer_config_from_meta(const ov::Model& model) {
    // Source the tokenizer metadata from the native GGUF FrontEnd rt-info schema (SPEC.md §3.3/§6.2)
...
create_tokenizer_from_config(const std::shared_ptr<void>& shared_object_ov_tokenizers,
    ...
    std::map<std::string, GGUFMetaData> tokenizer_config;
    // When the native GGUF FrontEnd is enabled (default), source the tokenizer config from the
    // ov::Model rt-info via Core::read_model instead of re-parsing the .gguf with the in-tree reader.
    // The in-tree reader only supports a few quant types (no Q5_K/IQ*/...), so re-parsing would fail
    // on such models even though the FE loads them fine (SPEC §6.2).
    if (utils::env_bool("OPENVINO_GENAI_USE_NATIVE_GGUF_FE", /*default=*/true)) {
